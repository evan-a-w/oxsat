...
let rec literal_of t ~clauses ~(formula : Formula_with_no_shared_theories.t) : int =
  match formula with
  ...
  | And fs ->
    let literals = List.map fs ~f:(fun formula -> literal_of t ~clauses ~formula) in
...
let formula_with_no_shared_theories t ~(formula : Formula.t)
  ...
  | Type
  | Type_of of Tvar.t
  | Type_app of Tvar.t * t
  (* Linear arithmetic *)
  | Scale_const of Q.t * t
  | Add of t * t
  | Const of Q.t
  | Compare of
