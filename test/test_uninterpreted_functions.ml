...
let%expect_test "transitivity conflict is unsat" =
  solve_with_uf ~build:(fun uf solver ->
    ...
    ignore
      (Solver.add_clause solver ~clause:[| eq_xy |] : [ `Ok | `Unsat of _ ]);
    ignore
...
let%expect_test "consistent equality and disequality is sat" =
  solve_with_uf ~build:(fun uf solver ->
    ...
    ignore
      (Solver.add_clause solver ~clause:[| eq_xy |] : [ `Ok | `Unsat of _ ]);
    ignore
      (Solver.add_clause solver ~clause:[| neq_yz |] : [ `Ok | `Unsat of _ ]));
...
let%expect_test "unregistered congruent applications conflict with disequality" =
  solve_with_uf ~build:(fun uf solver ->
    let x = Uf.new_const uf in
    let y = Uf.new_const uf in
    let fx = Uf.new_app uf ~func:0 ~args:[| x |] in
...
let%expect_test "nested unregistered congruent applications conflict with \
                 disequality"
  =
  solve_with_uf ~build:(fun uf solver ->
    let x = Uf.new_const uf in
    ...
    let fx = Uf.new_app uf ~func:0 ~args:[| x |] in
    let fy = Uf.new_app uf ~func:0 ~args:[| y |] in
    let gfx = Uf.new_app uf ~func:1 ~args:[| fx |] in
    let gfy = Uf.new_app uf ~func:1 ~args:[| fy |] in
    let eq_xy = Uf.register_eq uf ~lhs:x ~rhs:y in
    let neq_gfxgfy = Uf.register_neq uf ~lhs:gfx ~rhs:gfy in
    ignore
      (Solver.add_clause solver ~clause:[| eq_xy |] : [ `Ok | `Unsat of _ ]);
    ignore
      (Solver.add_clause solver ~clause:[| neq_gfxgfy |]
       : [ `Ok | `Unsat of _ ]));
  [%expect {| (Unsat (unsat_core (1))) |}]
;;

let%expect_test "congruence propagates registered disequality false" =
  solve_with_uf ~build:(fun uf solver ->
    let x = Uf.new_const uf in
    let y = Uf.new_const uf in
    let fx = Uf.new_app uf ~func:0 ~args:[| x |] in
    let fy = Uf.new_app uf ~func:0 ~args:[| y |] in
    let eq_xy = Uf.register_eq uf ~lhs:x ~rhs:y in
    ignore (Uf.register_neq uf ~lhs:fx ~rhs:fy : int);
    ignore
      (Solver.add_clause solver ~clause:[| eq_xy |] : [ `Ok | `Unsat of _ ]));
  [%expect {| (Sat (assignments (() (true) (false)))) |}]
;;

let%expect_test "negated neq behaves as equality" =
  solve_with_uf ~build:(fun uf solver ->
    let x = Uf.new_const uf in
    let y = Uf.new_const uf in
    let fx = Uf.new_app uf ~func:0 ~args:[| x |] in
    let fy = Uf.new_app uf ~func:0 ~args:[| y |] in
    let neq_xy = Uf.register_neq uf ~lhs:x ~rhs:y in
    let neq_fxfy = Uf.register_neq uf ~lhs:fx ~rhs:fy in
    ignore
      (Solver.add_clause solver ~clause:[| -neq_xy |] : [ `Ok | `Unsat of _ ]);
    ignore
      (Solver.add_clause solver ~clause:[| neq_fxfy |] : [ `Ok | `Unsat of _ ]));
  [%expect {| (Unsat (unsat_core (-1))) |}]
;;

let%expect_test "negated eq behaves as disequality" =
  solve_with_uf ~build:(fun uf solver ->
    let x = Uf.new_const uf in
    let y = Uf.new_const uf in
    let eq_xy = Uf.register_eq uf ~lhs:x ~rhs:y in
...
let%expect_test "repeated solve after adding theory clause has no stale reasons"
  =
  let uf = Uf.create () in
  let packed = Theory.Packed.create (module Uf) uf in
  let solver = Solver.create ~theory:packed () in
  ...
  print_s [%sexp (Solver.solve solver : Sat_result.t)];
  let neq_fxfy = Uf.register_neq uf ~lhs:fx ~rhs:fy in
  ignore
    (Solver.add_clause solver ~clause:[| neq_fxfy |] : [ `Ok | `Unsat of _ ]);
  print_s [%sexp (Solver.solve solver : Sat_result.t)];
...
let%expect_test "theory pop: backtrack restores consistency" =
  solve_with_uf ~build:(fun uf solver ->
    ...
    let neq_xz = Uf.register_neq uf ~lhs:x ~rhs:z in
    ignore
      (Solver.add_clause solver ~clause:[| eq_xy; eq_yz |]
       : [ `Ok | `Unsat of _ ]);
    ignore
      (Solver.add_clause solver ~clause:[| neq_xz |] : [ `Ok | `Unsat of _ ]));
...
(* Many terms forced into one equivalence class, then a disequality added at the
   end. The solver must backtrack repeatedly through many theory merges before
   concluding UNSAT, exercising pop on deep undo trails. *)
