...
let solve_clauses clauses =
  ...
  | Some core -> Sat_result.Unsat { proof = core }
...
let%expect_test "uniform random 3-SAT small instance" =
  let generator =
    Sat_generators.uniform_random_k_sat ~num_vars:5 ~num_clauses:10 ~k:3
  in
...
let%expect_test "hard 3-SAT near phase transition" =
  ...
  print_s [%message "Generated hard 3-SAT" (num_clauses : int)];
...
let%expect_test "planted solution is actually satisfiable" =
  let generator =
    Sat_generators.easy_planted_3_sat ~num_vars:10 ~num_clauses:20
  in
...
let%expect_test "scale-free distribution favors some variables" =
  let generator =
    Sat_generators.scale_free_k_sat ~num_vars:20 ~num_clauses:50 ~k:3 ~alpha:2.5
  in
  let clauses = Quickcheck.random_value generator in
...
let%expect_test "forced backbone constraints" =
  ...
  in
  print_s [%message "Backbone enforced with unit clauses" (unit_clauses : bool)];
  [%expect
    {|
...
let%test_unit "uniform k-SAT generates correct clause lengths" =
  Quickcheck.test
    ...
    ~f:(fun clauses ->
      ...
      Array.iter clauses ~f:(fun clause -> assert (Array.length clause = 3)))
;;
...
let%test_unit "solver handles scale-free instances" =
  Quickcheck.test
    ~sexp_of:[%sexp_of: int array array]
    ~trials:20
    (Sat_generators.scale_free_k_sat
       ~num_vars:12
       ~num_clauses:30
       ~k:3
       ~alpha:2.5)
        ...
        List.iter backbone ~f:(fun (var, expected_value) ->
          let var_assignment =
            Array.find_map assignments_array ~f:(fun lit ->
              if Int.abs lit = var then Some (lit > 0) else None)
          in
          ...
          | Some actual_value ->
            ...
            then
              failwithf
                "Backbone constraint violated: var %d should be %b but is %b"
                var
                expected_value
                actual_value
                ()
          | None -> failwithf "Backbone variable %d not assigned" var ())
      | Unsat _ ->
