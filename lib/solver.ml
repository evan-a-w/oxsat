...
module Reason = struct
  type t =
    | Decision of Literal.t
    | Clause_idx of int

  let decision literal = Decision literal
  let clause_idx idx = Clause_idx idx
end
...
let assignment t ~var =   if Bitset.get (Tf_pair.get t.assignments true) var
  then Some true
...
let learn_clause_from_failure ~failed_clause t =
  ...
  Trail_entry.Vec.iter_rev t.trail ~f:(fun trail_entry ->
    ...
    with
    | (true, _) -> ()
    | (false, Reason.Decision _) ->
      failwith "found decision walking back from conflict"
    | (false, Reason.Clause_idx clause_idx) ->
...
let on_new_var
  ...
  I64.Option.Vec.fill_to_length
    ...
    ~length:(var + 1);
  let fill tf with_ =     Tf_pair.iter
      tf
      ~f:
  ...
  fill watched_clauses_by_literal Int.H_set.create
;;
...
let update_watched_clauses t ~set_literal =
  ...
  let watched_clauses =
    Vec.Value.get
      ...
      (Literal.var literal)
  in
  let snapshot = Int.H_set.to_keys_array watched_clauses in
        ...
        then process_snapshot (idx + 1) acc
          ...
          let acc =
            let replace_with =
              ...
              |> List.find_local ~f:(fun searched_literal ->                 let literal' = Literal.of_int searched_literal in
...
let populate_watched_literals_for_new_clause
  ...
  let maybe_unit () =
    ...
    then ()
      ...
      Int.H_set.insert unprocessed_unit_clauses ~key:(Ptr.to_int ptr) ~data:()
  in
  let watch_literal (literal : int) =
    let bs =
      Vec.Value.get
        ...
        (Int.abs literal)
    in
    Int.H_set.insert bs ~key:(Ptr.to_int ptr) ~data:()
  in
...
let free_clause
  ...
  Clause.iter_literals clause ~f:(fun literal ->
    ...
    Int.H_set.remove (get_tang watched_clauses_by_literal) (Ptr.to_int ptr));
  Int.H_set.remove unprocessed_unit_clauses clause_idx;
...
let second_highest_decision_level t ~clause =
  let (_, max2) =
    ...
    |> List.fold_local
         ~init:(0L, 0L)
         ~f:(fun ((max1, max2) as acc) literal ->            match
...
let make_decision' ~is_assumption t ~literal : _ =
...
let make_decision t : _ =
  match (Vsids.choose_literal t.vsids : Literal.Option.t) with
  | None ->
    `Done
      (Sat_result.Sat
...
let check_sat_result t ~(sat_result : _) : _ =
...
let can_trim_clause t ~clause_idx =
  let clause = Clause.Pool.get t.clauses (Ptr.of_int clause_idx) in
  let literals = Clause.literals_list clause in
  let enough_literals =
    match literals with
    | [] | [ _ ] | [ _; _ ] | [ _; _; _ ] -> false
    | _ -> true
  in
  let rec enough_lbd (distinct_decision_levels) remaining_literals
    ...
    | literal :: remaining_literals ->
      (match
       ...
       | Some idx ->
         ...
         in
           (match
            List.find_local distinct_decision_levels ~f:(fun dl' ->
              if Int64.equal dl (I64.of_int dl') then Some () else None)
          with
...
let simplify_clauses t =
  ...
  Clause.Pool.iter t.clauses ~f:(fun ptr ->
    let clause_idx = Ptr.to_int ptr in
    if Int.H_set.mem t.learned_clauses clause_idx
       && (not (Int.H_set.mem t.clauses_with_active_unit clause_idx))
  ...
  for i = 0 to num_to_drop - 1 do
    ...
    Int.H_set.remove t.learned_clauses clause_idx
  done
;;

let rec try_unit_propagate t =   match
    (Int.H_set.choose_arbitrarily t.unprocessed_unit_clauses
     : Int.H_set.Kv_option.t)
  with
  | None -> None
  | Some pair ->
    let (clause_idx, _) = pair in
    Int.H_set.remove t.unprocessed_unit_clauses clause_idx;
    let clause = Clause.Pool.get t.clauses (Ptr.of_int clause_idx) in
    let literal = Clause.unit_literal clause ~assignments:t.assignments in
    (match (literal : Literal.Option.t) with
     | None -> try_unit_propagate t
     | Some literal ->
       if t.debug
       then
         print_s
...
let unsat t failed_clause_idx : Sat_result.t =
  (let failed_clause =
...
let rec solve' t : Sat_result.t =
  (t.iterations <- t.iterations + 1;
   ...
   let learn_from_failure failed_clause_idx : Sat_result.t =
     match
       I64.O.(
         t.decision_level = 0L
         || t.decision_level < t.decision_level_of_last_assumption)
     with
     | true -> unsat t failed_clause_idx
     | false ->
       let failed_clause =
         Clause.Pool.get t.clauses (Ptr.of_int failed_clause_idx)
       in
   ...
   | None ->
     (match make_decision t with
      | `Continue -> solve' t
      | `Failed_clause failed_clause_idx -> learn_from_failure failed_clause_idx
      | `Done sat_result -> check_sat_result t ~sat_result))
;;

let add_assumptions ~(assumptions) t =   let rec go i =     if i = Array.length assumptions
    then `Continue
    else (
      t.iterations <- t.iterations + 1;
      ...
      | None ->
        (match assignment t ~var:(Literal.var literal) with
         | None ->
           (match
              make_decision' ~is_assumption:true t ~literal
            with
         ...
         | Some b ->
           ...
           then go (i + 1)
             ...
             in
             match trail_entry.reason with
             | Reason.Decision _ -> failwith "invalid assumptions"
             | Reason.Clause_idx failed_clause_idx ->
               `Failed_clause failed_clause_idx)))
  in
  go 0
;;

let solve ?(assumptions = [||]) t : Sat_result.t =
  t.decision_level_of_last_assumption <- 0L;
  if t.debug
  then
    print_s
      [%message
        "solve" ~assumptions:(assumptions : int array)];
  if t.iterations > 0
  then (
    t.iterations <- 0;
    restart t);
  ...
  then Unsat { unsat_core = Clause.of_int_array [||] }
    ...
    match add_assumptions ~assumptions t with
    | `Continue -> solve' t
    | `Failed_clause failed_clause_idx -> unsat t failed_clause_idx)
;;
...
let create ?(debug = false) () =
  ...
  ; unprocessed_unit_clauses = Int.H_set.create ()
  ...
  ; vsids = Vsids.create ()
  ; learned_clauses = Int.H_set.create ()
