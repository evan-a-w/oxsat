module type S = sig
  ...
  (** Called at propagation quiescence. Returns:
      ...
      - [`Propagate of (int * int array) list] — list of [(literal,
        explanation_clause)] pairs the theory has deduced must hold *)
  val check_consistent
    :  t
    -> [ `Consistent | `Conflict of int array | `Propagate of (int * int array) list ]
