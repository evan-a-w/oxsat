...
let hard_instance ~num_vars =
  ...
  Instance.{ name = sprintf "Hard 3-SAT (n=%d, m=%d)" num_vars num_clauses; clauses }
;;
...
let easy_planted_instance ~num_vars ~num_clauses =
  ...
  Instance.{ name = sprintf "Planted 3-SAT (n=%d, m=%d)" num_vars num_clauses; clauses }
;;

let scale_free_instance ~num_vars ~num_clauses ~k ~alpha =
  let generator = Sat_generators.scale_free_k_sat ~num_vars ~num_clauses ~k ~alpha in
  let clauses = Quickcheck.random_value generator in
  Instance.
    { name = sprintf "Scale-free %d-SAT (n=%d, m=%d, alpha=%.2f)" k num_vars num_clauses alpha
    ; clauses
    }
;;
...
let planted_solution_instance ~num_vars ~num_clauses ~k ~min_satisfied =
  let generator =
    Sat_generators.planted_solution
      ~num_vars
      ~num_clauses
      ~k
      ~min_satisfied
...
let forced_backbone_instance ~num_vars ~num_clauses ~k ~backbone_size =
  let generator =
    Sat_generators.forced_backbone
      ~num_vars
      ~num_clauses
      ~k
      ~backbone_size
...
let random_k_cnf_instance ~num_vars ~num_clauses ~max_k =
  let generator =
    Sat_generators.random_k_cnf ~num_vars ~num_clauses ~max_k
  in
  let clauses = Quickcheck.random_value generator in
  Instance.
    { name = sprintf "Random k-CNF (n=%d, m=%d, max_k=%d)" num_vars num_clauses max_k
    ; clauses
    }
;;
...
let run_scaling_benchmark
  ...
  let uniform_instances =
    List.map sizes ~f:(fun n ->
      let num_clauses = Int.max (4 * n) (Int.of_float (4.2 *. Float.of_int n)) in
      uniform_instance ~num_vars:n ~num_clauses ~k:3)
  in
  let easy_planted_instances =
    List.map sizes ~f:(fun n ->
      let num_clauses = Int.max (3 * n) (Int.of_float (3.5 *. Float.of_int n)) in
      easy_planted_instance ~num_vars:n ~num_clauses)
  in
  let scale_free_instances =
    List.map sizes ~f:(fun n ->
      let num_clauses = Int.max (3 * n) (Int.of_float (3.7 *. Float.of_int n)) in
  ...
  let random_k_cnf_instances =
    List.map sizes ~f:(fun n ->
      let num_clauses = Int.max (4 * n) (Int.of_float (4.0 *. Float.of_int n)) in
